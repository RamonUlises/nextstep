...
export const CardTrabajos = ({
  ...
  return (
    <div className={`bg-gradient-to-br w-full rounded-2xl text-white font-medium relative md:max-w-[800px] sm:max-w-[550px] max-w-[400px] card ${nivel === 2 ? 'animate-shine' : 'to-[#1562AD] from-[#144678] dark:to-zinc-700 dark:from-zinc-800'}`} style={cardStyle}>
      <div className="container-card flex flex-col sm:flex-row items-center">
        ...
        <div className="sm:w-[75%] flex flex-col pt-6 pb-16">
          <div className="flex justify-end">
            <div className={` rounded-tl-3xl rounded-bl-3xl flex items-center py-3 px-6 gap-2 -mr-2 absolute sm:static top-0 right-0 mt-2 ${nivel === 2 ? 'bg-white text-black' : 'bg-principal-600 opacity-80 dark:bg-zinc-700'}`}>
              <Hourglass />
              <p>{fecha}</p>
            </div>
          </div>
